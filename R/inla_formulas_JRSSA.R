...
## with all covariates
formulas_all<- list()
for(i in 1:length(formulas)){ formulas_all[[i]]<- update(formulas[[i]],~.+x0+x1_stand+x2_stand+x3_stand+x4_stand+x5_stand+x6_stand) }
names(formulas_all)<- names(formulas)

## with x1, x5, and x6 
formulas_156<- list()
for(i in 1:length(formulas)){ formulas_156[[i]]<- update(formulas[[i]],~.+x1_stand+x5_stand+x6_stand) }
names(formulas_156)<- names(formulas)
